...
import { useGetItemsCartQuery, useGetRemoveItemMutation, useGetStoreBooksQuery, useGetTotalPriceQuery, useUpdataPriceMutation } from "../../services/api"
...
const ProductsListCart = () => {
    ...
    const { data: totalPrice, refetch: refetchTotalPrice } = useGetTotalPriceQuery()
    ...
    const handleChangeOption = (element: React.ChangeEvent<HTMLSelectElement>, idItem: number | undefined, price: number, quantBefore: number) => {
                ...
                <div className="container">
                    <ItemsOnCart>
                        ...
                        <ul className="books-list">
                            {
                                data && data.items.map(({ id, photo, price, name, quant }) => (
                                    <li key={id}>
                                        <div className="img-delete">
                                            <img src={photo} alt="item cart" />

                                        </div>
                                        <div className="name-text">{quant} x {name}</div>
                                        <div className="total-price">
                                            Valor: R$ {price},00
                                            <select onChange={(e) => handleChangeOption(e, id, price, quant)} className="quant" name="quant" value={quant}>
                        ...
                        <ButtonCart as="a" href="/checkout">Comprar Agora</ButtonCart>
                    </ItemsOnCart>
                    <span className="total">Preço total: R$ {totalPrice?.totalPrice}</span>
